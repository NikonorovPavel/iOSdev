...
class LogInView: UIView {
    ...
    private let defaultLogin = "nikonorov_pavel@icloud.com"
    ...
    private let passWarningLabel: UILabel = {
        let label = UILabel()
        label.text = "Password can't be less than 6 characters long"
        label.textColor = .red
        label.isHidden = true
    ...
    required init?(coder: NSCoder) {
        fatalError("init(coder:) has't been implemented")
    }
